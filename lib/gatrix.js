...
export default {
  ...
  getLocalUsername() {
    ...
    try {
      ...
      console.log('No Gatrix user info. Anynomous user.');
      return false;
    }
  },

  getLocalToken() {
    const localUsernamePath = `/.gatrix/refs/token`;
    const homedir = os.homedir();
    const usernameDir = path.join(homedir, localUsernamePath)
    try {
      const token = fs.readFileSync(usernameDir, 'utf8').replace(/\r?\n|\r/g, '');
      console.log('token', token);
      return token;
    } catch (e) {
      // console.log('No Gatrix user info.', e);
      console.log('No Gatrix token info. Anynomous user.');
      return false;
    }
  },
  ...
  init() {
    ...
    if (username && token) {
      ...
      this.fetchUser(username, token)
        .then((user) => {
          this.gatrixView.setUser(user)
          this.fetchUserRepositories(username, token)
            .then((repositories) => {
